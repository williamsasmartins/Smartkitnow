...
const bodyMetrics: Item[] = [
  { name: "BMI GÇö Body Mass Index Calculator", slug: "bmi-body-mass-index" },
  { name: "BMR GÇö Basal Metabolic Rate (Mifflin-St Jeor)", slug: "bmr-mifflin-st-jeor" },
  { name: "TDEE GÇö Total Daily Energy Expenditure Calculator", slug: "tdee-daily-energy-expenditure" },
...
const trainingPerformance: Item[] = [
  ...
  { name: "VOGéémax Estimator (Cooper/rockport)", slug: "vo2max-estimator-cooper-rockport" },
  { name: "1RM GÇö One-Rep Max (Epley/Brzycki)", slug: "one-rep-max-1rm-epley-brzycki" },
  { name: "Steps GåÆ Distance GåÆ Calories Converter", slug: "steps-distance-calories-converter" },
];
...
const womensHealth: Item[] = [
  ...
  { name: "TDEE Ajustado na Gesta+º+úo (educacional)", slug: "tdee-gestation-adjusted" },
];
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-7xl px-4 pb-16 lg:pr-[65px]">
        <div className="grid gap-8 lg:grid-cols-12">
          {/* coluna esquerda: header + conte+¦do */}
          <div className="lg:col-span-9 pr-[15px]">
            {/* HERO */}
            <header className="py-6 mb-8">
              <div className="flex items-center gap-3">
                <EmojiIcon symbol="G¥ñn+Å" size={38} className="text-primary" label="Health" />
              ...
              <div className="mt-4 max-w-4xl text-base md:text-lg leading-relaxed text-muted-foreground space-y-3">
                {descExpanded ? (
                  <>
                    <p>
                      Build a healthier routine with {TOTAL} practical calculators spanning body metrics, nutrition and macros, training & cardio, and womenGÇÖs health.
                    </p>
                    <p>
                    ...
                    <p>
                      Optimize performance: calculate <strong>running pace and splits</strong>, estimate <strong>calories burned</strong> via METs, determine <strong>heart-rate zones</strong>, approximate <strong>VOGéémax</strong>, compute <strong>1RM</strong>, and convert <strong>steps Gåö distance Gåö calories</strong>.
                    </p>
                    <p>
                      Support womenGÇÖs health: estimate <strong>ovulation window</strong>, calculate <strong>due date</strong>, track <strong>pregnancy weight gain</strong> by BMI, and review <strong>gestational TDEE</strong> concepts.
                    </p>
                  </>
                ) : (
                  <p className="line-clamp-3">
                    Build a healthier routine with {TOTAL} practical calculators spanning body metrics, nutrition and macros, training & cardio, and womenGÇÖs health. Track BMI, BMR and TDEE, estimate body fat, check waist-to-height, compute BSA, plan daily calories and set a realistic weight-loss timeline. Dial in macros with protein/carbs/fat splits, set protein, carb and fat targets (incl. AMDR & keto ranges), and plan fiber, water and balanced meal calorie distribution. Optimize performance: calculate running pace and splits, estimate calories burned via METs, determine heart-rate zones, approximate VOGéémax, compute 1RM, and convert steps Gåö distance Gåö calories. Support womenGÇÖs health: estimate ovulation window, calculate due date, track pregnancy weight gain by BMI, and review gestational TDEE concepts.
                  </p>
                )}
                {!descExpanded && (
                  <button
            ...
            <Section
              ...
              description="Compute BMI, BMR and TDEE; estimate body fat (US Navy/3-sites); check waist-to-height; calculate body surface area; plan daily calories and map a safe, timeGÇæbound weightGÇæloss strategy."
              items={bodyMetrics}
              base="/health"
            />
            ...
            <Section
              ...
              description="Plan macro distributions for protein, carbs and fat; set protein goals by objective (cut/bulk/maintain); determine carb ranges (lowGÇæcarb/keto); compute fat range via AMDR; target fiber and water intake; and split meal calories across the day."
            ...
            <Section
              ...
              description="Calculate running pace, speed and splits; estimate calories burned with METs; define heartGÇærate zones (Karvonen/percentages); estimate VOGéémax (Cooper/Rockport); compute oneGÇærep max (Epley/Brzycki); and convert steps to distance and calories."
            ...
            <Section
              emoji="=ƒæ¬GÇì=ƒì+"
              title={`WomenGÇÖs Health & Pregnancy (${womensHealth.length})`}
              description="Estimate ovulation and fertile window; calculate pregnancy due date (Naegele); plan recommended weightGÇægain range by BMI; and explore gestational TDEE considerations for educational use."
              items={womensHealth}
              base="/health"
            />
