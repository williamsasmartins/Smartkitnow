...
export default function MortgageAmortizationCalculator() {
  ...
  const calculations = useMemo(() => {
    ...
    if (loanAmount <= 0 || interestRate <= 0 || loanTerm <= 0) return { mainResult: 0, totalInterest: 0, totalPayment: 0, scheduleData: [] };

    const monthlyPayment = loanAmount * interestRate / (1 - Math.pow(1 + interestRate, -loanTerm));
    const totalPayment = monthlyPayment * loanTerm;
    const totalInterest = totalPayment - loanAmount;

    const scheduleData = Array.from({ length: loanTerm }, (_, i) => {
    ...
    return { mainResult: monthlyPayment, totalInterest, totalPayment, scheduleData };
  }, [inputs]);
  ...
  return (
    <CalculatorVerticalLayout
      ...
      example={{
        ...
        scenario: "Imagine you have a $200,000 mortgage at a 5% annual interest rate over 30 years.",
        steps: [
          { label: "Step 1", calculation: "5% / 12 = 0.004167", explanation: "Convert annual rate to monthly rate" },
          { label: "Step 2", calculation: "30 * 12 = 360", explanation: "Convert years to months" },
      ...
      relatedCalculators={[
        ...
        { title: "Debt-to-Income Calculator", url: "/financial/debt-to-income", icon: "💵" },
        { title: "Savings Goal Calculator", url: "/financial/savings-goal", icon: "🔢" },
      ]}
    >
