...
import {
  Search, Menu, X,
  HardHat, Car, DollarSign, Heart, ChefHat, RotateCcw,
  Calculator, Dog, Gamepad2, QrCode, Dumbbell, Zap,
  Atom, Clock, Home, Smile, TrendingUp, Video, Lightbulb, Quote,
} from "lucide-react";
...
export function Header() {
  ...
  return (
    ...
    >
      ...
      {mobileMenuOpen && (
        <nav className="sm:hidden border-t border-border/50 bg-background/98 shadow-lg max-h-[70vh] overflow-y-auto">
          {/* Primary categories */}
          <div className="px-4 pt-3 pb-1">
            ...
            <ul className="grid grid-cols-2 gap-1.5">
              {ALL_NAV_CATS.map((cat) => {
                const Icon = NAV_ICONS[cat.key];
                return (
                  <li key={cat.key}>
                    <Link
                      to={cat.to}
                      onClick={() => setMobileMenuOpen(false)}
                      className="flex items-center gap-2.5 px-2.5 py-2 text-sm font-medium text-foreground hover:bg-teal-50 dark:hover:bg-teal-900/20 hover:text-teal-700 dark:hover:text-teal-300 rounded-lg transition-colors"
                    >
                      {Icon ? (
                        <span className="w-7 h-7 rounded-lg bg-teal-50 dark:bg-teal-900/40 border border-teal-100 dark:border-teal-800 flex items-center justify-center shrink-0">
          ...
          {/* More categories */}
          <div className="px-4 pt-2 pb-3 border-t border-border/40 mt-2">
            <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/60 mb-2 px-1">
              More
            </p>
            <ul className="grid grid-cols-2 gap-1.5">
              {MORE_CATS.slice(0, 8).map((cat) => {
                const Icon = NAV_ICONS[cat.key];
                return (
                  <li key={cat.key}>
                    <Link
                      to={cat.to}
                      onClick={() => setMobileMenuOpen(false)}
                      className="flex items-center gap-2.5 px-2.5 py-2 text-sm font-medium text-foreground hover:bg-muted rounded-lg transition-colors"
                    >
                      {Icon ? (
          ...
          {[
            ...
            return (
              <li key={cat.key} className="flex items-center">
                <Link
                  to={cat.to}
                  className="text-primary hover:text-teal-600 dark:hover:text-teal-400 transition-colors inline-flex items-center gap-1.5 py-1 text-sm font-medium"
                >
                  {Icon ? <Icon className="w-3.5 h-3.5 shrink-0 opacity-70" aria-hidden /> : null}
                  {cat.label}
                </Link>
              </li>
            );
