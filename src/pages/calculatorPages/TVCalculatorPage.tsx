...
const TVCalculatorPage: React.FC = () => {
  ...
  const CalculatorComponent = lazy(() => /* @vite-ignore */ import(`@/components/calculators/tv/${componentName}.js`));

  return (
    <CalculatorLayout
      title={`${calcInfo.name} Calculator | Smart Kit Now`}
      description={`Use our ${calcInfo.name} calculator to ${calcInfo.description.toLowerCase()}. Accurate TV and video calculations.`}
      keywords={`${calcInfo.tags.join(', ')}, tv calculators, video tools, online calculator`}
